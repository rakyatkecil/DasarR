...
> Untuk melihat keseluruhan dari data (`data.frame`), gunakan fungsi `View(mydata)` (dengan `V` kapital). Setelah menjalankan fungsi tersebut pada `console` maka akan terbuka jendela baru yang menampilkan data secara keseluruhan.


## Ringkasan data dengan `summary()`
...
```{r}
rataan = mean(mydata$berat, na.rm = TRUE)
mydata$berat[is.na(mydata$berat)] = rataan
```
